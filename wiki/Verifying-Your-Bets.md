...
You can verify any **Drill** bet in three ways. All of them run **the same math the server used** —
nothing is trusted, everything is recomputed.

## What you need

From your Drill bet history, each bet shows:
...
[`paytable.json`](https://github.com/maczo-co/originals-drill/blob/main/paytable.json); the verifier
...
```
https://verify.maczo.co/originals/?game=drill&server=<revealed>&commit=<hash>&client=<you>&nonce=<n>&params=<url-encoded JSON>
```

## Method 2 — Browser, manual

1. Open https://verify.maczo.co/originals/?game=drill (works fully offline; view source).
...
**Your bet inputs (params).** This game takes: `target_e8`, `shape` — pass them as the `params` JSON, keys exactly as your bet history shows them.

## Method 3 — Node.js CLI

```bash
git clone https://github.com/maczo-co/originals-drill
cd originals-drill
