...
Each **Drill** result is a pure cryptographic function of a **server seed** (which we commit to *before*
...
`paytable.json` publishes exactly what maps the random words to a Drill outcome (multiplier tables /
...
Drill is **`modulo`** — reduces a uint32 by modulo, carrying a documented-negligible bias (< ~1e-5). Every game documents this so you know exactly how its words become an
outcome.
